...
//Code Here
let totalPopulation = populations.reduce((total, element) => total += element )
...
let ordersTotal = orders.map((total, element) => total += (element['price'] + element['tax']))
  ...
  newArr =[];
      for (let i=0; i<purchases.length; i++){
          if (purchases[i]["owner"]==="Bob"){
              newArr.push(purchases[i]["price"])
      }
  } return newArr;
}

var bobArray = bobFunc()

let bobsTotal = bobArray.reduce((total, element) => total += element)
